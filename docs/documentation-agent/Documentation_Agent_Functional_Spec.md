...
For a source-grounded edit bounded to one established target claim, the resulting proposal shall add or correct semantic information rather than merely restate a subset of the claim. An insertion shall not expose a private paired helper absent from the target or create standalone response-detail prose for an identifier already named by the target. If multiple established claims share a section, the proposal anchor shall uniquely identify one candidate after whitespace and Markdown table-boundary normalization; an absent, unmatched, or ambiguous anchor shall not select a claim. Every accepted insertion shall use the resolved established claim as its `INSERT_AFTER` anchor. A replacement may narrow an established paragraph to the stripped model anchor only when that anchor is an exact substring of the established claim and occurs exactly once in the current document; otherwise the full established claim shall remain the boundary. If an established claim contains three or more distinct inline-code contract values, a replacement shall preserve at least half of them; otherwise the proposal is skipped as destructively incomplete. These guards do not reject a concise correction or material insertion that introduces a new factual value.
