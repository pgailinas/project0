...
For a source-grounded edit bounded to one established target claim, the resulting proposal shall add or correct semantic information rather than merely restate a subset of the claim. An insertion shall not expose a private paired helper absent from the target or create standalone response-detail prose for an identifier already named by the target. Every accepted insertion shall use the unique established claim as its `INSERT_AFTER` anchor. If an established claim contains three or more distinct inline-code contract values, a replacement shall preserve at least half of them; otherwise the proposal is skipped as destructively incomplete. These guards do not reject a concise correction or material insertion that introduces a new factual value.
