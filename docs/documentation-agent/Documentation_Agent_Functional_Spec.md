...
Stage 1 `documentation_gap_analysis` output shall contain summary, gaps, assumptions, and warnings. Each gap identifies its target document, optional section, description, source evidence, and optional confidence. Exact semantic duplicates are removed. For an HTTP endpoint return claim, analysis shall ground the contract in returned payload fields rather than docstring wording. A claimed contradiction is rejected when deterministic inspection establishes that at least two compound returned-dictionary keys named by the exact target claim are present; a claim for an absent payload field is not rejected by this guard. No verified gaps completes the workflow without Stage 2 or proposals.

Stage 2 shall make one `documentation_update` request using a compact context containing the requested change, each exact target claim, its verified gap, source evidence, and bounded rewrite rules. It shall not receive unrelated repository context or introduce new gaps or design changes. The complete source-grounded context shall remain available to deterministic proposal validation. When configured, `strict-documentation-editor` is loaded for Stage 2 only. Ordinary mode issues one `documentation_update` request with built-in Markdown-only, minimum-change, style-preservation, no-invention, source-read-only, and target-scope constraints when no skill is active.

For a source-grounded edit bounded to one established target claim, the resulting proposal shall add or correct semantic information rather than merely restate a subset of the claim. The model shall supply concrete replacement wording, while the workflow shall assign the sole verified exact claim as the anchor even when model output omits or invents an anchor. An insertion shall not expose a private paired helper absent from the target or create standalone response-detail prose for an identifier already named by the target. If multiple established claims share a section, the proposal anchor shall uniquely identify one candidate after case, whitespace, and Markdown table-boundary normalization; an absent, unmatched, or ambiguous anchor shall not select a claim. Every accepted insertion shall use the resolved established claim as its `INSERT_AFTER` anchor. A replacement may narrow an established paragraph to the stripped model anchor only when that anchor is an exact substring of the established claim and occurs exactly once in the current document; otherwise the workflow-owned full established claim shall remain the boundary. Wording that combines fail-closed resolution with continuing a selection or edit shall be rejected. If an established claim contains three or more distinct inline-code contract values, a replacement shall preserve at least half of them; otherwise the proposal is skipped as destructively incomplete. These guards do not reject a concise correction or material insertion that introduces a new factual value.
