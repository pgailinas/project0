...
Strict-mode filtering occurs before review presentation. Exact-claim replacements or insertions that merely restate existing information are omitted. Replacements that discard most values from an established inline-code contract enumeration, insertions that expose private implementation helpers, and standalone response-detail insertions for identifiers already named by the claim are likewise omitted. When several established claims share a section, the proposal anchor must uniquely identify the affected claim after whitespace and Markdown table-boundary normalization. Accepted insertions appear directly after the resolved verified claim. An exact unique replacement anchor limits the displayed and applied difference to its verified subclaim, preserving surrounding paragraph text; otherwise the full established claim is used. Filtered changes are reported through page warnings; if another valid proposal remains, the page stays in review-required state and presents that proposal with the warnings.
