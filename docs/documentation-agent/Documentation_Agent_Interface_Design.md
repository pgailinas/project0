...
Strict-mode filtering occurs before review presentation. Exact-claim replacements or insertions that merely restate existing information are omitted. Replacements that discard most values from an established inline-code contract enumeration, insertions that expose private implementation helpers, and standalone response-detail insertions for identifiers already named by the claim are likewise omitted. Accepted insertions appear directly after the verified claim. Filtered changes are reported through page warnings; if another valid proposal remains, the page stays in review-required state and presents that proposal with the warnings.
