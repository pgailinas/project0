...
Strict-mode filtering occurs before review presentation. Exact-claim replacements or insertions that merely restate existing information are omitted. Replacements that discard most values from an established inline-code contract enumeration, insertions that expose private implementation helpers, and standalone response-detail insertions for identifiers already named by the claim are likewise omitted. When Stage 1 supplies one verified claim for a proposal path, the workflow assigns that exact claim as the review and application anchor even if model output omitted or invented one. When several claims remain candidates, the supplied anchor must uniquely identify the affected claim after case, whitespace, and Markdown table-boundary normalization. Accepted insertions appear directly after the resolved verified claim. An exact unique replacement subclaim limits the displayed and applied difference while preserving surrounding paragraph text; otherwise the workflow-owned full established claim is used. Filtered changes are reported through page warnings; if another valid proposal remains, the page stays in review-required state and presents that proposal with the warnings.
