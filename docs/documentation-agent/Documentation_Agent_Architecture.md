...
Source-grounded mode reads only requested targets and sources, labels them `TARGET DOCUMENTATION` or `AUTHORITATIVE SOURCE`, and aborts on any read failure. It performs gap analysis, exact-gap deduplication, and bounded proposal generation for established gaps. Stage 2 receives a compact rewrite context containing the request, exact target claim, verified gap, and source evidence rather than the complete repository context; the complete context remains available to deterministic proposal validation. `strict-documentation-editor` is used in proposal generation when configured. Ordinary mode uses Knowledge Service context and one update-reasoning call without automatic baseline inclusion.
...
Structured output is parsed before deterministic enforcement, and enforcement precedes review. Eligible proposals update existing `.md` files within repository and allowlist boundaries. Source-grounded proposals receive additional concrete-content, fenced-Python, declaration-fidelity, unique-location/anchor, semantic-alignment, exact-claim redundancy, implementation-detail, and contract-enumeration preservation checks. A replacement or insertion that only restates information already present in its exact target claim is filtered before review. Insertions also reject newly exposed private source helpers and standalone response-detail assertions for identifiers already named by the claim. When exactly one established claim is known for a proposal path, the workflow assigns that verified claim as the anchor and does not depend on the model to reproduce it. When several established claims remain candidates, a supplied anchor must uniquely match one claim after case, whitespace, and Markdown table-boundary normalization; missing, unmatched, or ambiguous resolution fails closed. Accepted exact-claim insertions are anchored immediately after the verified claim rather than at a broad section boundary. For replacement, an exact unique model subclaim may narrow an established paragraph only when it occurs exactly once so surrounding same-line prose is preserved; otherwise the workflow-owned full established claim remains the boundary. A replacement also fails closed when it would discard most inline-code values from an established enumeration. Contradictory wording that combines fail-closed resolution with continuing a selection or edit is rejected. For HTTP endpoint return claims, bounded analysis is instructed to use returned payload fields rather than docstring wording, and a narrow deterministic guard rejects a claimed contradiction when the target names at least two fields that are present in a returned dictionary.
