...
Structured output is parsed before deterministic enforcement, and enforcement precedes review. Eligible proposals update existing `.md` files within repository and allowlist boundaries. Source-grounded proposals receive additional concrete-content, fenced-Python, declaration-fidelity, unique-location/anchor, semantic-alignment, exact-claim redundancy, implementation-detail, and contract-enumeration preservation checks. A replacement or insertion that only restates information already present in its exact target claim is filtered before review. Insertions also reject newly exposed private source helpers and standalone response-detail assertions for identifiers already named by the claim. When a section contains multiple established claims, a model-provided anchor must uniquely match one claim after whitespace and Markdown table-boundary normalization; missing, unmatched, or ambiguous anchors do not select a claim. Accepted exact-claim insertions are anchored immediately after the verified claim rather than at a broad section boundary. For replacement, an exact unique model anchor may narrow an established paragraph to its verified subclaim so surrounding same-line prose is preserved; an unverified or non-unique anchor leaves the full established claim as the boundary. A replacement also fails closed when it would discard most inline-code values from an established enumeration. For HTTP endpoint return claims, bounded analysis is instructed to use returned payload fields rather than docstring wording, and a narrow deterministic guard rejects a claimed contradiction when the target names at least two fields that are present in a returned dictionary.
