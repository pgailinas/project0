...
- **Fallback title enrichment** reads the embedded title from an acquired canonical seed PDF and uses it only to replace an unresolved `arXiv:<identifier>` placeholder. Ordinary provider titles remain authoritative and unchanged.
