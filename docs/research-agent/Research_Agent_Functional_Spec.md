...
When evidence acquisition downloads a canonical fallback seed PDF with a usable embedded document title, that authoritative title shall replace the `arXiv:<identifier>` placeholder in both paper metadata and its source reference. Provider-supplied titles are never overwritten, and absent, generic, identifier-only, or malformed PDF titles leave the placeholder unchanged.
