...
│
├── site/                 # Generated by mkdocs gh-deploy
│
├── skills/
│   └── <skill-name>/
│       └── SKILL.md
...
| `site/` | Generated GitHub Pages documentation output created by Material for MkDocs. This directory is not the authoritative documentation source. |
| `skills/` | Repository-local Project0 Agent Skills organized by skill directory using `SKILL.md` definitions. |
| `src/` | Python implementation of reusable Project0 platform services, artifact services, the Dashboard Framework, shared infrastructure, and specialized AI agents. |
...
Repository-local Agent Skills are stored under `skills/<skill-name>/SKILL.md` and are available as reusable platform capabilities for applicable Project0 agents.
